...
config_path = r"Files/config/time_manager/config.ini"
...
class MainWindow(QMainWindow):
    ...
    def setUpMainWindow(self):
        ...
        phone_button.setIcon(QIcon(r"Files/icons/to phone.png"))
        ...
        #Goal deadlines reminder
        #self.reminder()

    # def reminder(self):
    #     #load all the goals with set deadlines
    #     #check the condition
    #     #If true - open a window that shows these goals
    #     def urgent(priority, deadline):
    #         days_to_deadline = QDate.fromString(QDate.currentDate().toString("yyyy-MM-dd"), "yyyy-MM-dd").daysTo(QDate.fromString(deadline, "yyyy-MM-dd"))
    #         if priority == "A" and days_to_deadline < 30:
    #             return True
    #         if priority == "B" and days_to_deadline < 20:
    #             return True
    #         if priority == "C" and days_to_deadline < 10:
    #             return True
    #         if priority == "D" and days_to_deadline < 5:
    #             return True
    #         return False

    #     #Download goals with deadlines and make a list of urgent ones
    #     goals = DataManager.loadMainData("goalsdeadlines")
    #     p_categories = ["A", "B", "C", "D"]
    #     urgent_goals = []
    #     for category in p_categories:
    #         urgent_goals += [goal for goal in goals if urgent(goal[2], goal[3])]
    ...
    def task_time_expired(self):
        self.notifier.notify(f"Time of {self.title_edit.text()} task expired.", title="Time Manager")
...
2. Timer\n
    ...
    def send_notification(self):
        ...
        if not note:
            note = "(no note)"
        self.notifier.notify(note, title="Time Manager", subtitle=title)
