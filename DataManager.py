...
def loadMainData(data_type, *args, one=False):
    ...
    if data_type == "goalsdeadlines":
        cur.execute("SELECT ID, name, priority, limit_date FROM Goals WHERE state != 'completed'")
...
def loadOtherData(data_type, *args, one=False):
    ...
    if data_type == "author image":
        cur.execute("SELECT image FROM Authors WHERE name == ?", args)

    if data_type == "phrase author":
        cur.execute("SELECT author FROM Phrases WHERE name = ?", (args[0],))

    if data_type == "phrases for the day":
