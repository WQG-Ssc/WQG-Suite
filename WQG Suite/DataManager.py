...
def updateMainData(data_type, args):
    ...
    if data_type == "goal":
        cur.execute("UPDATE Goals SET name = ?, total_difficulty = ?, time = ?, benefit = ?, limit_date = ?, priority = ?, used_skills = ?, state = ?, note = ?, files = ?, progress = ?, custom_characteristics = ? WHERE ID == ?", args)
    conn.commit()
    conn.close()
