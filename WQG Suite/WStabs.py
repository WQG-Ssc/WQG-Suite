    ...
    def updateWidget(self):
        ...
        for goal_info in goals:
            ...
            if goal_info[10]:
                ...
                for i in range(2, 8 + len(self.ccs)):#ѕотом последние значение будет получатс€ по кол-ву характеристик
    ...
    def save_goal(self, previous=None):
        ...
        if (goal_name and len(characts) == 4 and used_skills and full_cc_values and skills_valid) or (is_group and goal_name and len(characts) > 2):
            if self.goals_dict[self.current_goal_id].isGoalExists:
                if is_group:
                    if len(characts) < 4:#¬алидность определ€етс€ по длине списка, поэтому вставить значение 0 изначально нельз€
                        characts.insert(0, 0)
